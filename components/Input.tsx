...
const Input: React.FC<FormField> = ({ name, field, type, disabled, label, placeholder }) => {
	return (
		<div className={'min-h-12 mb-3'}>
            {field && type != "checkbox" ? <label htmlFor={name} className="mb-1 block text-sm text-gray-500">
				{label}
			</label> : null}
			<input
				className={clsx([
				...
				disabled={disabled}
			/>
            {type == "checkbox" ? <label htmlFor={name} className="mb-1 text-sm text-gray-600">
				{label}
			</label> : null}
		</div>
	)
